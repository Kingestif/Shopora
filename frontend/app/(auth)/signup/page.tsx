    ...
    <Card className="border-none shadow-2xl bg-white/80 backdrop-blur-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className={`text-[10px] font-medium transition-colors ${isPasswordValid ? 'text-green-500' : 'text-slate-400'}`}>
                {password.length > 0 && (isPasswordValid ? "Length OK" : `Min. ${minLength} characters`)}
              </span>
            </div>
            <Input 
              id="password" 
              ...
              className={`rounded-xl h-12 transition-all ${
                password.length > 0 && !isPasswordValid ? 'border-red-400 focus-visible:ring-red-400' : 'border-slate-200'
              }`} 
              required 
            />
          </div>
      ...
      <CardFooter className="flex flex-col gap-4">
        ...
        <div className="text-[10px] text-center w-full text-slate-400 uppercase tracking-widest">
            Joining as a {role} 
            <Link href={`/signup?role=${isSeller ? 'BUYER' : 'SELLER'}`} className="ml-2 underline">
